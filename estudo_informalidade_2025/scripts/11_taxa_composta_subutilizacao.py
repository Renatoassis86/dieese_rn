# -*- coding: utf-8 -*-
# Taxa Composta de Subutilização — Barras agrupadas por local (BR, NE, RN) e ano (2019→2025)
# Padrão visual/metodológico:
# - 1 figura com 3 grupos no eixo X (BR, NE, RN)
# - 7 barras por grupo (anos 2019..2025), 2025 = T2; 2019..2024 = T4
# - Paletas distintas (tons de azul) por local
# - Rótulos percentuais acima das barras (1 casa)
# - Legenda inferior (anos 19..25), sem título
...
def _safe_read_csv(path_csv: str) -> pd.DataFrame:
    """Leitura robusta com auto-mapeamento e suporte a CSV com ';'."""
    def read_any(p):
        try:
    ...
    # Caso venha tudo numa única coluna separada por ';'
    ...
    c_taxa    = find_col("tx_composta", "taxa", "tx_composta_subutilizacao")
...
def _pick_annual_row(g: pd.DataFrame, ano: int) -> pd.Series:
    """Escolhe T4 (<=2024) e T2 (2025) como representativos."""
...
def _to_uniform(df: pd.DataFrame) -> pd.DataFrame:
    """Tabela anual 2019..2025 por local (apenas BR/NE/RN)."""
...
def main(path_csv: str = CSV_DEFAULT):
    ...
    assert_min_rows(df, MIN_ROWS, "taxa composta de subutilização (barras)")
    assert_percentage_bounds(df["taxa"], "tx_composta")
    ...
    # 5) plot — paletas distintas por local
    for gi, loc in enumerate(FOCO_LOCAIS):
        ...
        for yi, ano in enumerate(anos):
            ...
            color = palette[yi]
            bar = ax.bar(xpos, val, width=bar_w*0.96, color=color, edgecolor=color, linewidth=0.9,
                         zorder=2, label=year_labels[yi] if gi == 0 else "_nolegend_")[0]
            if not np.isnan(val):
                ax.text(bar.get_x() + bar.get_width()/2.0, val + 0.6, _fmt_pct(val),
    ...
    # 7) legenda inferior (anos 19..25; referência visual na paleta do BR)
    legend_handles = [Line2D([0],[0], color=PALETAS["Brasil"][i], lw=10) for i in range(len(year_labels))]
    fig.legend(handles=legend_handles, labels=year_labels, ncol=7, loc="lower center", bbox_to_anchor=(0.5, 0.04))
    ...
    if ax.get_xlabel():
        raise RuntimeError("Eixo X não deve ter xlabel.")

    # 9) exporta
    outdir = PROJECT_DIR / "outputs" / "figs"
    outdir.mkdir(parents=True, exist_ok=True)
    outfile = outdir / "11_tx_composta_subutilizacao.png"
